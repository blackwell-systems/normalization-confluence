...
oscillate.  We then remove the tree restriction altogether: admitting any
acyclic network in which a multi-source target carries a \emph{resolution
operator}---a source-determined, validity-preserving merge of its incoming
constraints---we recover the same convergence guarantee, with single-source
authority as the special case.  Because the merge encodes domain policy rather
than mathematics, the guarantee is conditioned on two decidable properties of
the operator, checkable by finite enumeration.  For self-containment, we restate
the single-registry model and
...
We then lift the tree restriction to any acyclic network by equipping
multi-source targets with \emph{resolution operators}---source-determined,
validity-preserving merges that generalize the single-source authority
function---so convergence holds across arbitrary directed-acyclic constraint
graphs.
...
theorem with necessity counterexamples, and its generalization to multi-source
acyclic networks via resolution operators (\Cref{sec:multisource}).  For
...
\item \textbf{Multi-source resolution} (\Cref{sec:multisource}): Removal of the
  ...
  determinacy and validity preservation (R1--R2), convergence holds on any
  acyclic network (\Cref{thm:resolved-convergence}), with the tree theorem as
  the single-source special case.  Both conditions are decidable by finite
  enumeration, so a resolved network is certified exactly as WFC and CC are.
\item \textbf{Monotone cycles} (\Cref{sec:monotone}): When shared domains are
  ordered lattices and repair is monotone, acyclicity is unnecessary---federated
  convergence holds on \emph{any} network (\Cref{thm:monotone-cycles}) by the
  Knaster--Tarski theorem, with order-independence identified as chaotic
  iteration.  CRDTs are the compensation-free special case, and acyclicity is
  revealed as a concession required only for non-monotone repair.
\item \textbf{Compositionality} (\Cref{sec:compositionality}): A convex,
  ...
  registry} (\Cref{thm:collapse}); the outer network converges iff the collapsed
  one does.  This gives modular and hierarchical verification---and black-box
  reuse of verified subsystems---generalizing product composition from
  independent registries to arbitrary sub-diagrams (limits compose).
\item \textbf{Machine-checked base theorem} (\Cref{sec:mechanization}): The
  single-registry Convergence Theorem this work builds on is mechanized in
  Coq/Rocq---Newman's Lemma and the three critical-pair cases---axiom-free
  (\texttt{Print Assumptions} reports ``Closed under the global context''), with
  a concrete registry witnessing that WFC and CC are jointly satisfiable.
\end{enumerate}
...
again axiom-free with a concrete witness.  Scripts are in the \texttt{coq/}
directory of the artifact.  Not yet mechanized: the full chaotic-iteration
statement (that every fair \emph{asynchronous} component-wise schedule, not only
the synchronous iteration, reaches~$\bar{s}^{\star}$).
\end{remark}
...
resolution operators---in \Cref{sec:multisource}, recovering convergence on any
acyclic network.
\end{remark}
...
\[
  R_j : \textstyle\prod_{i \in S(j)} \Sigma_{R_i} \longrightarrow S_j
\]
from the source states to $j$'s shared component, satisfying:
\begin{enumerate}[leftmargin=*,nosep,label=(R\arabic*)]
\item \textbf{Source determinacy}: $R_j$ is a function of the source states
...
\item \textbf{Validity preservation}: for all $(\sigma_i)_{i \in S(j)}$ with
  ...
  \[
    V_{R_j}\!\left(\sigma_j[\mathrm{sh} \mapsto
      R_j((\sigma_i)_{i \in S(j)})]\right) = \top .
  \]
\end{enumerate}
When $|S(j)| = 1$, taking $R_j(\sigma_i) = \phi_{ij}(\sigma_i)$ recovers the
...
with $|S(j)| > 1$ carries a resolution operator $R_j$; single-source targets
...
\[
  ...
  \pi_j^{\mathrm{sh}}(\sigma_j) = R_j((\sigma_i)_{i \in S(j)}) .
\]
\end{definition}
...
$\pi_j^{\mathrm{sh}}(\sigma_j) \neq R_j((\sigma_i)_{i \in S(j)})$, set
$\sigma_j \mapsto \sigma_j[\mathrm{sh} \mapsto R_j((\sigma_i)_{i \in S(j)})]$.
...
On a resolved network, a single application of $\rho_\Fed$ produces a federally
valid state.
\end{lemma}

\begin{proof}
Identical to \Cref{lem:fed-termination}, with the single-source repair
$\phi_{ij}(\sigma_i)$ replaced by $R_j((\sigma_i)_{i \in S(j)})$ and M1 by (R2).
...
Let $\mathcal{N}$ be an acyclic resolved network in which every single-source
morphism satisfies M1, every resolution operator satisfies (R1)--(R2), and every
component registry satisfies WFC and CC.  Then any two processors consuming the
same federated events converge to the same federally valid state.
\end{theorem}

\begin{proof}
The three-component decomposition of \Cref{lem:authority} generalizes.
\emph{(a)}~Source registries (in-degree zero) are untouched by resolution repair
and equal their single-registry normal forms.  \emph{(b)}~In a federally valid
state, a target~$j$ satisfies
$\pi_j^{\mathrm{sh}}(\sigma_j) = R_j((\sigma_i)_{i \in S(j)})$
(or $\phi_{ij}(\sigma_i)$ when $|S(j)|=1$)---a deterministic function of its
sources' normal forms.  \emph{(c)}~Its local component is untouched by
resolution (by (R1) the operator writes only the shared component) and converges
by component CC.

By induction on the topological order, each component's normal form is a
deterministic function of the per-component event multisets.  \emph{CC2}: for a
federally invalid $\bar{\sigma}$ and event~$e$, source normal forms are fixed by
component CC2 or unaffected; each shared component is a deterministic function
(via $R_j$ or $\phi$) of source normal forms, which agree across the two paths;
local components converge by component CC2.  \emph{CC1}: independent events
targeting distinct registries affect disjoint per-component multisets, and each
shared component is recomputed from source normal forms, so order is immaterial;
independent events on a common registry commute by component CC1.  Hence the
federated system satisfies CC.  \Cref{lem:resolved-termination} gives WFC with
the trivial measure $\Phi_\Fed \in \{0,1\}$, so \Cref{thm:convergence} yields a
unique normal form. \qed
\end{proof}

\begin{corollary}[Constructive Resolved Normal Form]
\label{cor:resolved-nf}
\Cref{cor:fed-nf} extends: compute each source's single-registry normal form,
then, in topological order, set each target's shared component by evaluating its
resolution operator (or authority morphism) on its finalized sources, and let
each target converge its local component by component CC.  As in the tree case,
the product state space is never materialized.
\end{corollary}
...
(R1)--(R2), exactly as the single-source case is conditioned on M1.  Both
conditions are decidable---the source and target state spaces are finite, so
(R1) and (R2) are checked by enumerating
$\prod_{i \in S(j)} \Sigma_{R_i} \times \Sigma_{R_j}$ per multi-source
target---so a build-time verifier certifies a resolved network exactly as it
certifies WFC and CC for a single registry.  \Cref{thm:fed-convergence} is the
special case in which every target has a single source whose resolution operator
is the morphism image.
\end{remark}
...
is (\Cref{prop:m1-necessary}): a resolver mapping a valid source combination to
a shared component that violates $V_{R_j}$ induces oscillation between resolution
repair and local compensation.  (R1) is necessary for well-definedness: were
$R_j$ to read the local component, the shared normal form would depend on the
order in which local compensation and resolution repair interleave, defeating
single-round termination.
\end{remark}
...
domain, acyclicity is unnecessary: convergence follows from the
Knaster--Tarski fixed-point theorem, and order-independence is exactly the
classical convergence of \emph{chaotic iteration}.
...
(least element $\bot_j$; all joins exist).  With the component local normal forms
fixed (order-independent by component CC), define the \emph{federated repair
operator} $\Phi : \prod_j S_j \to \prod_j S_j$ by
\[
  \Phi(\bar{s})_j =
  \begin{cases}
    s_j^{\mathrm{ev}}
      & \text{$j$ a source (no incoming): its event-determined shared value,}\\[2pt]
    \phi_{ij}\big(\sigma_i[\mathrm{sh}\mapsto s_i]\big)
      & \text{$j$ single-source from $i$,}\\[2pt]
    R_j\big((\sigma_i[\mathrm{sh}\mapsto s_i])_{i \in S(j)}\big)
      & \text{$j$ multi-source.}
  \end{cases}
\]
...
(componentwise).  Source entries are constant maps, hence monotone; the
condition constrains only the morphism images and resolvers.
\end{definition}
...
satisfy WFC and CC.  Then $\Phi$ has a least fixed point
$\bar{s}^{\star} = \bigsqcup_{k \geq 0} \Phi^k(\bot)$, and every fair
asynchronous (any-order) iteration of the component updates from $\bot$ converges
to $\bar{s}^{\star}$.  Consequently all processors consuming the same federated
events converge to the same federally valid state, with no acyclicity
requirement.
\end{theorem}

\begin{proof}
$\Phi$ is a monotone self-map of the complete lattice $\prod_j S_j$ (a product of
complete lattices), so by Knaster--Tarski~\cite{tarski1955} it has a least fixed
point, obtained by Kleene iteration as
$\bar{s}^{\star} = \bigsqcup_k \Phi^k(\bot)$---the ascending chain
$\bot \sqsubseteq \Phi(\bot) \sqsubseteq \cdots$ stabilizes (for a finite lattice
in at most its height times $|\mathcal{I}|$ steps).  Order-independence is the
convergence of chaotic iteration: for a monotone operator on a complete lattice,
any fair schedule of component updates started from $\bot$ reaches the same least
fixed point~\cite{cousot1977}.  Validity is preserved along the iteration by
M1/R2 exactly as in \Cref{lem:resolved-termination}, so $\bar{s}^{\star}$ is
federally valid.  With the local components fixed by component CC, the full
federated normal form is determined; hence the system is confluent. \qed
\end{proof}

\begin{corollary}[Acyclicity is a concession to non-monotonicity]
\label{cor:acyclicity-monotone}
Acyclicity is needed only to tame non-monotone repair.  The cyclic divergence of
\Cref{prop:cycle-necessary} uses the antitone negation $\phi(x) = 1-x$; under
monotonicity that operator is excluded and \Cref{thm:monotone-cycles} applies.
Acyclic structure and monotonicity are thus two independent routes to federated
confluence---the former for arbitrary (possibly non-monotone) repair, the latter
for arbitrary (possibly cyclic) topology.
\end{corollary}

\begin{remark}[Order-independence \emph{is} chaotic iteration]
\Cref{thm:monotone-cycles} exposes the central property of normalization
confluence in another guise: convergence ``regardless of application order'' is
precisely the order-independent convergence of chaotic iteration in dataflow
analysis and abstract interpretation~\cite{cousot1977}.  The monotone regime
inherits that machinery---worklist scheduling, and widening/narrowing for tall or
infinite lattices---for computing and accelerating the federated normal form.
\end{remark}
...
validity-preserving, so a state-based CRDT~\cite{shapiro2011crdt} is the special
case of \Cref{thm:monotone-cycles} in which repair never has to restore an
invariant.  Normalization confluence adds compensation on top: monotone
convergence \emph{with} business invariants, which operation commutativity alone
does not provide.
\end{remark}

\begin{remark}[Infinite lattices and widening]
\label{rem:infinite-lattice}
\Cref{thm:monotone-cycles} assumes only a \emph{complete} lattice, which may be
infinite.  Knaster--Tarski still yields the least fixed point $\bar{s}^{\star}$;
what changes is its computation.  Kleene iteration reaches $\bar{s}^{\star}$ in
finitely many steps under the ascending chain condition (in particular, for any
finite lattice).  Without ACC---an unbounded counter or resource in
$\mathbb{N} \cup \{\top\}$, whose ascending chains never stabilize---iteration
need not terminate; a \emph{widening} operator (abstract
interpretation~\cite{cousot1977}) forces termination by extrapolating the
ascending chain to a sound post-fixed-point, trading exactness for
computability.  The convergence guarantee is unaffected: the federated normal
form exists and is unique regardless; only its exact computation requires ACC or
a widening.
\end{remark}
...
morphisms satisfy M1/R2 and its components satisfy WFC and CC; acyclic, or
cyclic with monotone repair).  Then:
\begin{enumerate}[leftmargin=*,nosep,label=(\alph*)]
\item $R_{\mathcal{J}}$ satisfies WFC and CC.
...
\item Let $\mathcal{N}'$ replace the sub-federation by $R_{\mathcal{J}}$.  Then
  $\mathcal{N}'$ is a well-formed network, acyclic if $\mathcal{N}$ is, and
  $\mathcal{N}$ converges iff $\mathcal{N}'$ does; their normal forms agree under
  $\Sigma_{R_{\mathcal{J}}} = \prod_{i \in \mathcal{J}} \Sigma_{R_i}$.
\end{enumerate}
\end{theorem}

\begin{proof}
\emph{(a)}~WFC: $\rho_{\Fed}^{\mathcal{J}}$ drives the sub-federated measure to a
federally valid state (\Cref{lem:fed-termination}, \Cref{lem:resolved-termination}),
so it is a well-founded compensation for $R_{\mathcal{J}}$.  CC: internal
convergence (\Cref{thm:fed-convergence}, \Cref{thm:resolved-convergence}, or
\Cref{thm:monotone-cycles}) makes the sub-federated normal form a deterministic
function of the events targeting $\mathcal{J}$ and the import-port values; CC1
(order independence of internal events) and CC2 (absorption of internal
compensation) are precisely that confluence read through $R_{\mathcal{J}}$.
...
obligation---overwriting with a valid source image preserves target
validity---is unchanged, since sub-federated re-normalization preserves federal
validity by~(a).  An export edge reads $R_{\mathcal{J}}$'s normalized state, a
...
$\mathcal{N}$ is.  With (a)--(b), $\mathcal{N}'$ meets the hypotheses of the
federated convergence theorem.  Because $\mathcal{J}$ is convex, a topological
order of $\mathcal{N}$ visits it as a contiguous block, so $\rho_{\Fed}$ on
$\mathcal{N}$ factors as ``normalize the sub-federation, then propagate across
the boundary''---which is exactly $\rho_{\Fed}$ on $\mathcal{N}'$ with
$R_{\mathcal{J}}$'s compensation inlined.  Hence the normal forms coincide. \qed
\end{proof}
...
replace it by its effective registry, and verify the network of effective
registries.  The construction nests---giving hierarchical verification---and a
verified sub-federation is reusable as a black-box component with a fixed port
interface.  \Cref{thm:product} is the special case $E_{\mathcal{J}} = \varnothing$
with empty boundary (independent registries).
\end{corollary}

\begin{remark}[Convexity and the monotone regime]
Convexity is needed only to keep the collapsed network acyclic.  In the monotone
regime (\Cref{thm:monotone-cycles}) a boundary-induced cycle is harmless: if the
sub-federation's repair is monotone then so is $\rho_{\Fed}^{\mathcal{J}}$, and
\Cref{thm:monotone-cycles} applies to $\mathcal{N}'$ regardless of convexity.
\end{remark}
...
\emph{independent} routes to convergence (\Cref{cor:acyclicity-monotone}).  When
repair is \emph{monotone} on ordered shared domains, convergence holds on
\emph{any} topology---including cyclic meshes---by \Cref{thm:monotone-cycles}.
When repair may be \emph{non-monotone} (toggles, cancellations, order-reversing
revocations), the topology must be \emph{acyclic}, with resolution operators
(\Cref{sec:multisource}) at multi-source targets, and convergence is the one-shot
constructive normal form (\Cref{cor:resolved-nf}).  In both regimes repair must
preserve validity (M1/R2).  \Cref{tab:regimes} summarizes the trade; the two
axes---monotonicity and acyclicity---are orthogonal, and either alone suffices.

\begin{table}[ht]
\centering
\small
\begin{tabular}{@{}lp{3.1cm}p{3.0cm}p{4.2cm}@{}}
\toprule
\textbf{Regime} & \textbf{Repair requirement} & \textbf{Topologies} & \textbf{Convergence guarantee} \\
\midrule
Monotone &
Monotone on a lattice order (join \emph{or} meet); validity-preserving &
Any: cyclic meshes, DAGs, trees &
Least fixed point $\mathrm{lfp}(\bot)$ under any \emph{fair} asynchronous order (Knaster--Tarski + chaotic iteration) \\[4pt]
Non-monotone &
...
Deterministic one-shot constructive normal form; feedback loops structurally excluded \\
\bottomrule
\end{tabular}
\caption{Two regimes for federated repair.  Monotonicity and acyclicity are
independent conditions for confluence: monotone repair converges on any topology
(\Cref{thm:monotone-cycles}); non-monotone repair requires acyclicity
...
organizational boundaries.  When a supplier answers to \emph{several}
...
the nondeterminism that would otherwise require additional conditions.  When a
target answers to several sources at once, a \emph{resolution operator}
generalizes the authority function---a source-determined, validity-preserving
merge---and convergence extends to any acyclic network, with single-source
authority as the special case.  Because the merge encodes domain policy, its two
conditions are stated as decidable hypotheses (checkable by finite enumeration)
rather than assumed, mirroring how the single-source case rests on morphism
validity preservation.  Finally, when the shared domains are ordered lattices and
repair is monotone, even \emph{cyclic} networks converge---by the
Knaster--Tarski fixed-point theorem---which reveals acyclicity as a requirement
only for non-monotone repair, exhibits order-independent convergence as chaotic
iteration, and places state-based CRDTs as the compensation-free special case.
Federated convergence is moreover \emph{compositional}: a convex, internally
convergent sub-federation collapses to a single effective registry, so large
networks verify modularly and hierarchically and verified subsystems compose as
