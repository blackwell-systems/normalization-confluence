...
basis of the nerve (first-Betti-number many loops), and coordinating those loops suffices (the
minimum coordinated set is analyzed in 10.3):
...
(first-Betti-number many loops) suffices. It is not in general the MINIMUM number of coordinated
edges (one edge can break several cycles); that minimum, and its complexity, are analyzed in 10.3.
*Proof sketch (sufficiency).* Coordinating a loop forces its shared variable to a single agreed
value, which is the single-writer overlap of section 10, so that loop's transition becomes a
coboundary and its holonomy generator dies. Killing a generating set of `H^1` leaves the trivial
...
**The complexity, resolved: this is Group Feedback Edge Set.** The minimum coordination is exactly the
**group feedback edge set** problem on the holonomy-labeled nerve: delete the fewest edges so every
residual cycle is balanced (trivial holonomy). This is a studied problem, and identifying it settles
the complexity, including a correction to an earlier claim of mine.

- *NP-hard, even in the abelian case.* For `G = Z/2` the problem is edge bipartization (delete fewest
  edges to make the graph bipartite), equivalently the complement of Max Cut, which is NP-hard. So the
  minimum is NP-hard even when the holonomies commute. This CORRECTS an earlier suggestion of mine
  that the abelian case is a polynomial matroid rank: the rank counts independent obstructing
  directions, not the edges needed to hit them, and since one edge lies on many cycles the
  edge-minimum is the harder bipartization number, not the rank. (The rank is only a lower bound.)
- *Hard to approximate.* Under the Unique Games Conjecture, edge bipartization has no constant-factor
  approximation; the best known is `O(√log n)` (the Min-Uncut bound). So there is no cheap
  near-optimal coordination in general.
- *Fixed-parameter tractable in the core size `k`.* Group feedback vertex/edge set is FPT in the
  solution size (Cygan-Pilipczuk-Pilipczuk for the cutset parameterization; iterative compression for
  edge bipartization). This is the practically relevant tractability: the coordinated core is
  something you WANT small (few consensus groups), and when it is small the exact minimum is
  computable in `f(k)·poly(n)`.
- *Polynomial special cases.* Planar nerves (edge bipartization, hence Max Cut, is polynomial on
  planar graphs); and edge-disjoint obstruction, where one deleted edge per obstructing cycle is
  forced and sufficient, so the minimum is exactly the number of obstructing cycles for any `G`. The
  edge-disjoint case is the independent-loop result mechanized as `simultaneous_section_iff`, with
  the algorithm: run the diagnostic on a cycle basis and coordinate the orbiting cycles.
...
And the abelian case is not even a computational shortcut: it is already NP-hard (edge bipartization),
so abelianizing neither sounds the sizing nor makes it tractable.

**What to deploy, given the complexity.** The exact minimum is NP-hard, but the accept-with-
coordination capability is still deployable, in a tiered way that matches the resolved landscape:

- A correct, polynomial, `≤ b` coordination is always available: pick any spanning tree, run the
  diagnostic on each fundamental cycle, and coordinate the ones that orbit (every residual
  fundamental cycle is then trivial, so a section exists). It is exactly minimal in the edge-disjoint
  regime; otherwise it is a valid over-approximation bounded by the Betti number.
- When the coordinated core is meant to be small (the desirable regime, few consensus groups), run
  the FPT algorithm in `k` to get the exact minimum.
- On planar nerves, solve it exactly in polynomial time (planar edge bipartization).
- Otherwise fall back to the `O(√log n)` approximation, with the understanding that a constant-factor
  guarantee is Unique-Games-hard.
